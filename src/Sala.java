...
public class Sala{
	...
	public Item getItem(String item){
		for(Item valoresDaLista: itens){
			if(valoresDaLista.getClass().getName().toLowerCase().equals(item)){
	...
	public void mostraPortas(){
		for(int i = 0; i < 3; i++){
			if(portas[i].getParaQualSala() != -1){
				System.out.print("Porta: "+(char)(i+65)+" ");
			}
		}
		System.out.println();
	}
	...
	public void mostraItem(){
		...
		System.out.print("Machado(s): "+qntMachado);
		System.out.println(" | Poção(s): "+qntPocao);
		System.out.println(" | Chave(s): "+qntChave);
	}
}
